...
trap_init(void)
{
	...
	extern uint8_t INT0[], INT1[], INT3[], INT4[], INT5[], INT6[], INT7[], INT8[], INT9[], INT10[], INT11[],
			INT12[], INT13[], INT14[], INT16[], INT48[];

	// LAB 3: Your code here.
	SETGATE(idt[0], 1, GD_KT, INT0, 0);
	SETGATE(idt[1], 1, GD_KT, INT1, 0);
	// skip 2
	SETGATE(idt[3], 1, GD_KT, INT3, 3);
	...
	SETGATE(idt[T_SYSCALL], 1, GD_KT, INT48, 3);
...
trap_dispatch(struct Trapframe *tf)
{
	...
	switch (tf->tf_trapno) {
		...
		case T_SYSCALL: {
			...
			int r = syscall(reg->reg_eax, reg->reg_edx, reg->reg_ecx, reg->reg_ebx, reg->reg_edi, reg->reg_esi);
...
page_fault_handler(struct Trapframe *tf)
{
	...
	// LAB 3: Your code here.
	if ((tf->tf_cs & 0x3) == 0) {
		panic("Kernel has a page fault %x", fault_va);
	}
	...
	struct UTrapframe* utrapf = (void *)UXSTACKTOP;
	if (tf->tf_esp >= UXSTACKTOP-PGSIZE && tf->tf_esp < UXSTACKTOP) {
		utrapf = (void *)(tf->tf_esp);
	}
	utrapf = (struct UTrapframe *)((uint32_t)utrapf - sizeof(int) - sizeof(struct UTrapframe));
	user_mem_assert(curenv, utrapf, sizeof(struct UTrapframe) + sizeof(int), PTE_W);
