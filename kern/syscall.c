...
sys_cputs(const char *s, size_t len)
{
	...
	/* if (user_mem_check(curenv, s, len, PTE_U) < 0) {
		cprintf("In syscall_cputs, read from %x to %x\n", s, s+len);
		cprintf("Destory the environment...\n");
		env_destroy(curenv);
	*/
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	pp_src = page_lookup(srcenv->env_pgdir, srcva, &pte_src);
	if (!pp_src) {
		return -E_INVAL;
	}
	...
	ret = page_insert(dstenv->env_pgdir, pp_src, dstva, perm);
	if (ret < 0) {
		return -E_NO_MEM;
	}
